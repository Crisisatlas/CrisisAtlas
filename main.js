...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Función para mostrar el mapa y los marcadores
  function mostrarmap() {
      ...
      var otherSections = document.querySelectorAll('.us-container, .notice-container, .crisis-container, .section-destacadas');
  ...
  // Función para buscar un país en el mapa y centrarlo
  function buscarPais() {
      var input = document.getElementById('search-input').value.toLowerCase();
      var paisEncontrado = countriesData.find(function(country) {
          return country.name.toLowerCase() === input;
      });

      if (paisEncontrado) {
          // Mostrar el mapa y centrarlo en el país encontrado
          mostrarmap();
          map.setView(paisEncontrado.coordinates, 5);
          // Abrir el panel con la información del país encontrado
          abrirPanel(paisEncontrado.name, paisEncontrado.news);
      } else {
  ...
  function showSuggestions(suggestions) {
    ...
    suggestions.forEach((suggestion) => {
      const suggestionHTML = `<div class="suggestion" data-name="${suggestion.name}">${suggestion.name}</div>`;
      suggestionsContainer.innerHTML += suggestionHTML;
    });
    suggestionsContainer.style.display = 'block';

    // Agregar evento click a las sugerencias
    const suggestionsDivs = document.querySelectorAll('.suggestion');
  ...
  var botonBuscar = document.querySelector('.search-icon');
  ...
  // Añadir evento al campo de búsqueda para buscar un país al presionar Enter
  ...
  var countriesData = [
    ...
    },
    {
      ...
      news: [
        ...
        {
          title: "Inminente fin del parole en EE. UU. y regreso de nicaragüenses ",
          source: "100% noticias",
          url: "https://www.laprensani.com/2024/07/15/economia/3346697-vencimiento-del-plazo-del-parole-provocara-retorno-masivo-que-impactara-en-la-economia",
          summary: "Investigación realizada por Hagamos Democracia advierte sobre la fuerte dependencia que las familias e incluso el Estado han creado con las remesas, por lo que el retorno de beneficiarios del parole impactará en la economía de las familias y del país"
        },
        {
          title: "Pronostican ambiente caluroso para esta semana, a pesar del ingreso de dos ondas tropicales",
          source: "EL PAIS",
          url: "https://www.laprensani.com/2024/07/15/nacionales/3347754-pronostican-ambiente-caluroso-para-esta-semana-a-pesar-del-ingreso-de-dos-ondas-tropicales",
          summary: "A pesar de que durante esta semana se espera que las ondas tropicales números 14 y 15 ingresen al territorio nacional, las lluvias esperadas son de “débiles a moderadas” por lo que es muy probable que los nicaragüenses vuelvan a experimentar un ambiente caluroso, producto de las altas y bajas presiones —asociadas a la zona de convergencia intertropical, localizada entre el suroeste del Mar Caribe y el Sur de Nicaragua— sumado a la influencia de los efectos del aire seco del Polvo del Sahara."
        },
      ]
    },
    {
...
,
 ...
 // Añadir evento al campo de búsqueda para mostrar sugerencias mientras se escribe
   document.getElementById("search-input").addEventListener("input", sugerenciaBusqueda);
